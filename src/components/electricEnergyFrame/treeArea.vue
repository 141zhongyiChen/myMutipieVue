<template>
  <div id="area-wrapper" @contextmenu="showMenu">
    <!-- 区域树 -->
    <div ref="area">
...
import { getTreeDataOfElectricEnergy, getCheckBoxFlag } from 'api/getDataOfElectricEnergy'
      ...
      ChildRender: {
        render: (h, params) => {
          ...
          switch (data.objData.type) {
            ...
            case 'EnergyMeter_NS':
              filter = 'addressCode'
              bgFilter = this.treeAddressCode
              break
            default:
              filter = 'terminalCode'
              bgFilter = this.treeTerminal
          }
          return h('span', {
            style: {
  ...
  methods: {
    ...
    click_layer (data) {
      ...
      if (data.length > 0) {
        ...
        this.setAddressCode(data[0].objData.addressCode)
      }
      console.log(this.treeAddressCode)
    },
    ...
    selectCurrentHover () { // 勾选当前鼠标移入的终端
      ...
      switch (arr[0].objData.type) {
        case 'EnergyMeter_ZL':
        case 'EnergyMeter_NS':
          this.setAddressCode(arr[0].objData.addressCode)
    ...
    quitAll (index) { // 取消所有
      ...
      switch (ret[0].objData.type) {
        case 'EnergyMeter_ZL':
        case 'EnergyMeter_NS':
          this.setAddressCode(ret[0].objData.addressCode)
    ...
    _getTreeData () {
      ...
      // 开发环境，默认开发人员编码：030000000000000000000007
      if (process.env.NODE_ENV === 'development') {
        getTreeDataOfElectricEnergy(data).then((res) => {
          let { regionTreeData } = JSON.parse(res)
          // 为当前对象每一项数据，添加render函数
          regionTreeData.map((rItem) => {
            // 为父节点添加 render 函数
          ...
          this.currentPageData = regionTreeData
          // checkedArr监测,需要在创建时更新数据
          let checkedArr = this.treeCheckedArr
          if (checkedArr.length > 0) {
            this.dataList = InitCheckedArr(regionTreeData, checkedArr, this.filterCode)
