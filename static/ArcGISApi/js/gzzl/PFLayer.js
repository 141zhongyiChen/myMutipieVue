define(["dojo/_base/declare", "esri/layers/tiled"], function(declare) {
	return declare(esri.layers.TiledMapServiceLayer, {
		...
		getPath: function() {
			...
			if(path.indexOf('127.0.0.1:8020') >= 0) {
				path = 'http://localhost:58643';
			}
			return path;
		},
		...
		getTileUrl: function(level, row, col) {
			...
			if(this.cache == false) {
				parm = "active=MapTiles_Uitls.GetTiles&level=" + level + "&col=" + col + "&row=" + row + "&UniqueCode=" + this.UniqueCode + "&ElementCode=" + this.ElementCode + "&x=" + this.generateMixed(5);
			} else {
				parm = "active=MapTiles_Uitls.GetCacheTiles&level=" + level + "&col=" + col + "&row=" + row + "&UniqueCode=" + this.UniqueCode + "&ElementCode=" + this.ElementCode;
			}

			var url = this.getPath() + "/LInterface/DataHandler.ashx?" + this.EncryptDES(parm);
			return url;
		}
	});
});